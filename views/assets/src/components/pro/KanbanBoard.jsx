  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[280px]" data-pm-dialog>
        <DialogHeader>
          <DialogTitle>{__("Background Color")}</DialogTitle>
        </DialogHeader>
          ...
          {COLOR_SWATCHES.map((color) => (
            <button
              ...
              className={cn(
                ...
                currentColor === color
                  ? "border-pm-text-primary ring-2 ring-pm-accent"
                  : "border-transparent",
              )}
        ...
        <Button
          variant="ghost"
          size="sm"
          className="w-full mt-1 text-xs text-pm-text-muted"
          onClick={() => {
            onSelect("");
            onOpenChange(false);
          }}
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md" data-pm-dialog>
        <DialogHeader>
          <DialogTitle>{__("Import Task")}</DialogTitle>
        </DialogHeader>
        <div className="space-y-3">
          {loadingLists ? (
            <p className="text-xs text-pm-text-muted">{__("Loading...")}</p>
          ) : (
            <div className="space-y-1.5">
              ...
              >
                <SelectTrigger className="h-8 text-sm">
                  <SelectValue placeholder={__("Select Task List")} />
                </SelectTrigger>
                <SelectContent>
            ...
            <>
              <Tabs value={tab} onValueChange={setTab}>
                <TabsList className="h-8">
                  ...
                  <TabsTrigger value="complete" className="text-xs px-3 h-6">
                    {__("Completed")}
                  </TabsTrigger>
                  <TabsTrigger value="incomplete" className="text-xs px-3 h-6">
                    {__("Incomplete")}
                  </TabsTrigger>
                </TabsList>
              </Tabs>
              ...
              <ScrollArea className="h-48 border rounded-md">
                {loadingTasks ? (
                  <div className="p-4 text-xs text-center text-pm-text-muted">
                    {__("Loading...")}
                  </div>
                ) : filtered.length === 0 ? (
                  <div className="p-4 text-xs text-center text-pm-text-muted">
                  ...
                  <div className="p-2 space-y-1">
                    {filtered.map((t) => (
                      <div
                        key={t.id}
                        className="flex items-center gap-2 py-1 px-1 rounded hover:bg-muted/30"
                      >
                        <Checkbox
                          id={`kt-${t.id}`}
...
function AutomationModal({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md" data-pm-dialog>
        <DialogHeader>
          <DialogTitle>{__("Manage Automation")}</DialogTitle>
        </DialogHeader>
          ...
          <div className="space-y-5 pr-3">
            ...
            <div className="space-y-2">
              ...
              {moveType === "none" && (
                <p className="text-xs text-pm-text-muted italic">
                  {__("This column will not be automated")}
                </p>
              )}
              ...
              {moveType === "todo" && (
                <div className="space-y-3 pl-1">
                  <Label className="text-xs font-medium">
                    {__("Move task here when...")}
                  </Label>
                  <RadioGroup
                    value={todoSection}
                    onValueChange={setTodoSection}
                    className="space-y-2"
                  >
                    <div className="space-y-1">
                      ...
                      <p className="text-[11px] text-pm-text-muted pl-6">
                        {__(
                          "Tasks added recently on any task lists will be automatically moved here.",
                        )}
                      </p>
                    </div>
                    <div className="space-y-1">
              ...
              {moveType === "done" && (
                <div className="space-y-2 pl-1">
                  ...
                  <p className="text-[11px] text-pm-text-muted pl-6">
                    {__(
                      "Issues will automatically move here when marked as complete.",
                    )}
                  </p>
                </div>
              )}
            ...
            <div className="space-y-2">
              ...
              <p className="text-[11px] text-pm-text-muted">
                {__(
                  "Select team members to be assigned automatically when a task dropped in this column.",
                )}
              </p>
            ...
            <div className="space-y-2">
              ...
              >
                <div className="space-y-1">
                  ...
                  <p className="text-[11px] text-pm-text-muted pl-6">
                    {__("Dropping task here has no progress status")}
                  </p>
                </div>
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
        ...
        <DialogFooter>
          ...
          <Button size="sm" onClick={handleSave} disabled={saving}>
            {saving ? __("Saving...") : __("Update Automation")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
...
function SearchAddTask({ projectId, boardId, onAdd }) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      api
        .get(`projects/${projectId}/tasks`, { title: query, per_page: 10 })
        ...
        <div className="absolute top-8 left-0 w-56 bg-white border rounded-lg shadow-lg z-10 max-h-48 overflow-y-auto">
          {searching ? (
            ...
            results.map((t) => (
              <button
                key={t.id}
                className="w-full text-left px-3 py-2 text-sm hover:bg-muted/30 truncate"
                onMouseDown={() => handleAdd(t.id)}
              >
                {t.title}
              </button>
            ))
    ...
    <div className="bg-white rounded-lg p-2 group">
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-start gap-1.5 flex-1 min-w-0">
          ...
          <h4 className="text-sm font-medium text-pm-text-primary leading-snug flex-1">
            {task.title}
          </h4>
        </div>
        <button
          ...
          className="opacity-0 group-hover:opacity-100 p-0.5 rounded hover:bg-red-50 text-pm-text-muted hover:text-red-500 transition-all shrink-0 border-none outline-none shadow-none bg-transparent"
          title={__("Remove")}
        >
      ...
      <div className="flex items-center gap-2 mt-2 text-xs text-pm-text-muted">
        {dueDate && (
          <span
            className={cn("flex items-center gap-1", overdue && "text-red-500")}
          >
    ...
    <>
      <div
        className={cn(
          ...
          dragOver
            ? "border-pm-accent border-2 bg-pm-accent/5"
            : "border-pm-border/50 bg-muted/20",
        )}
        onDragOver={(e) => {
          e.preventDefault();
        ...
        onDrop={(e) => {
          ...
          try {
            ...
            if (data.taskId && data.fromBoardId !== board.id)
              onMoveTask(data.taskId, data.fromBoardId, board.id);
          } catch {}
        }}
      >
        {/* Header */}
        <div
          className="px-3 py-2.5 rounded-t-xl flex items-center justify-between gap-1"
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button className="p-1 rounded hover:bg-black/10">
                  <MoreVertical className="h-4 w-4 text-pm-text-muted" />
                </button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="w-48">
                <DropdownMenuItem onClick={() => setAutomationOpen(true)}>
                  <Settings className="h-3.5 w-3.5 mr-2" />
                  {__("Manage Automation")}
                </DropdownMenuItem>
        ...
        <ScrollArea className="flex-1 px-2 py-2 max-h-[calc(100vh-240px)]">
          <div className="space-y-2 min-h-[40px]">
            ...
            {tasks.length === 0 && dragOver && (
              <div className="h-16 rounded-lg border-2 border-dashed border-pm-accent/40 flex items-center justify-center text-xs text-pm-accent">
                {__("Drop here")}
              </div>
            )}
          </div>
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// KANBAN DND BOARD (Kibo-powered drag-drop wrapper)
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
function KanbanDndBoard({
  ...
  // Transform boards to Kibo columns format
  ...
  // Transform tasks to Kibo data format (flat list with column reference)
  ...
  // Handle Kibo drag-drop data change
  const handleDataChange = useCallback((newData) => {
    setKanbanData(newData);
  }, []);

  // Handle new task created — add to kanbanData without full reload
  ...
  // Handle drag end — sync to API silently (UI already updated by onDataChange)
  const handleDragEnd = useCallback(
    (event) => {
      ...
      if (newColumnId && String(newColumnId) !== String(item.boardId)) {
        // API call in background — no reload needed, Kibo already moved the card
        // Get task order in the destination column
    ...
    [kanbanData, projectId, proApi, __, onRefresh],
  );
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// KANBAN BOARD COLUMN (single column with header + cards)
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    ...
    <>
      <KanbanBoardDnd id={column.id}>
        ...
        <KanbanHeaderDnd className="!p-0">
          <div
            className="flex items-center justify-between w-full rounded-t-md px-3 py-2"
            style={{
              backgroundColor: headerBg || "#f1f5f9",
              color: headerBg ? "#fff" : "inherit",
            }}
          >
            ...
            <div className="flex items-center gap-0.5 shrink-0">
              <button
                className="p-0.5 rounded hover:bg-white/20 border-none outline-none shadow-none bg-transparent"
                onClick={() => setAddingTask(true)}
              >
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button className="p-0.5 rounded hover:bg-white/20 border-none outline-none shadow-none bg-transparent">
        ...
        <div className="p-2 border-t border-border/50">
          {addingTask ? (
            ...
            <button
              className="w-full text-left text-sm text-muted-foreground hover:text-foreground p-1 rounded hover:bg-muted flex items-center gap-1.5"
              onClick={() => setAddingTask(true)}
            >
              <Plus className="h-3.5 w-3.5" />
              {__("Add New Task")}
...
export default function KanbanBoard() {
  ...
  // Load project users for filter/automation
  useEffect(() => {
    if (!projectId) return;
    api
      .get(`projects/${projectId}`, { with: 'assignees' })
      .then((res) => {
        const p = res?.data ?? res;
        const assignees = p?.assignees?.data || [];
        setUsers(Array.isArray(assignees) ? assignees : Object.values(assignees || {}));
      })
      .catch(() => {});
  }, [projectId]);
  ...
  const handleCreateBoard = async () => {
    ...
    try {
      ...
      toast.success(__("Section created"));
      loadAllBoards();
    } catch {
      toast.error(__("Failed to create section"));
    }
  };
  ...
  const handleDeleteBoard = (boardId) => {
    if (window.confirm(__("Delete this column?")))
  ...
  const handleMoveTask = useCallback(
    async (taskId, fromBoardId, toBoardId) => {
      if (!toBoardId) {
        ...
        try {
          ...
          toast.success(__("Task removed"));
          loadAllBoards();
        } catch {
          toast.error(__("Failed to remove task"));
        }
        return;
      }
      // Move between boards
    ...
    [projectId, proApi, __, loadAllBoards],
  );
        ...
        if (Array.isArray(filteredBoards)) {
          filteredBoards.forEach((fb) => {
            ...
            dispatch({
              ...
              payload: { boardId: fb.id, tasks: Array.isArray(tasks) ? tasks : [] },
            });
          });
        }
      } catch {
  ...
  return (
    <div className="max-w-[1900px] mx-auto p-6 h-full">
      {/* Top bar */}
